...
double gaussQuadrature(double (*points)[4]) {
    double surface = 0., dxdKSI, dydKSI, dxdETA, dydETA;
    double scales[2] = { 1, 1 };
    double nodes[2] = { -0.5773502692, 0.5773502692 };
    double derivative_ksi[2][4], derivative_eta[2][4], FUN_DETJ[2][2];

    for (int i = 0; i < 2; i++) {
        derivative_ksi[i][0] = -0.25 * (1. - nodes[i]);
        derivative_ksi[i][1] = 0.25 * (1. - nodes[i]);
        derivative_ksi[i][2] = 0.25 * (1. + nodes[i]);
        derivative_ksi[i][3] = -0.25 * (1. + nodes[i]);

        derivative_eta[i][0] = -0.25 * (1. - nodes[i]);
        derivative_eta[i][1] = -0.25 * (1. + nodes[i]);
        derivative_eta[i][2] = 0.25 * (1. + nodes[i]);
        derivative_eta[i][3] = 0.25 * (1. - nodes[i]);
    }

    // Liczenie pochodnych cz¹stkowych
    for (int i = 0; i < 2; i++) {
